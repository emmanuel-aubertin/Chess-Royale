...
class Pawn(p):
    ...
    def is_legal(self, board, pos):
        ...
        if (board_list[pos[0]][pos[1]] and board_list[pos[0]][pos[1]].team == self.team):
            return False
        print(str(pos[0]) + " == " + str(self.pos[0] - 1) + "and " + str(pos[1]) + "== " + str(
            self.pos[1]) + " and  not " + str(board_list[pos[0]][pos[1]]) + " |||| " + str())
        if ((pos[0] == self.pos[0] - 1 and pos[1]) == self.pos[1] and not board_list[pos[0]][pos[1]]):
            return True
        if ((pos[0] == self.pos[0] + 1 and pos[1]) == self.pos[1] and not board_list[pos[0]][pos[1]]):
            return True
        if (pos[0] == self.pos[0] + 1 and pos[1] == self.pos[1] + 1 and board_list[pos[0]][pos[1]] and
                board_list[pos[0]][pos[1]].team != self.team):
            return True
        if (pos[0] == self.pos[0] - 1 and pos[1] == self.pos[1] - 1 and board_list[pos[0]][pos[1]] and
                board_list[pos[0]][pos[1]].team != self.team):
            return True
    ...
    def get_playable_pos(self, board):
        ...
        if (self.team == 0):
            ...
            if (board_list[self.pos[0] - 2][self.pos[1]] == None and board_list[self.pos[0] - 1][self.pos[1]] == None and self.pos[0] == 6):
                print("4")
        ...
        if (self.team == 1):
            ...
            if (board_list[self.pos[0] + 1][self.pos[1] - 1] != None and board_list[self.pos[0] + 1][self.pos[1] - 1].team != self.team):
                print("7")
                pos_list.append([self.pos[0] + 1, self.pos[1] - 1])
            if (board_list[self.pos[0] + 2][self.pos[1]] == None and board_list[self.pos[0] + 1][self.pos[1]] == None and self.pos[0] == 1):
                print("8")
                pos_list.append([self.pos[0] + 2, self.pos[1]])
        return pos_list
